...
def test_asian_call_positive(S: float, K: float, sigma: float, steps: int):
    ...
    result = monte_carlo_asian_option(
        S, K, 0.05, sigma, 1.0, steps, "call", 1000, seed=42
    )
    assert result["price"] >= 0
...
def test_asian_put_positive(S: float, K: float, sigma: float, steps: int):
    ...
    result = monte_carlo_asian_option(
        S, K, 0.05, sigma, 1.0, steps, "put", 1000, seed=42
    )
    assert result["price"] >= 0
...
def test_barrier_cheaper_than_vanilla(S: float, barrier: float, sigma: float):
    ...
    vanilla_result = monte_carlo_option_price(
        S, 100, 0.05, sigma, 1.0, "call", 2000, seed=42
    )
...
def test_retirement_mean_positive(
    initial: float, contribution: float, return_rate: float
):
...
def test_retirement_ruin_probability_range(
    initial: float, withdrawal: float, volatility: float
):
