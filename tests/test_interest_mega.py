...
import math
import pytest
from finlib.interest import simple_interest, compound_interest, continuous_compound_interest, apr_to_apy, apy_to_apr, effective_interest_rate, future_value, present_value, real_interest_rate, doubling_time, compound_interest_earned
PRINCIPALS = [100, 5000, 100000]
...
@pytest.mark.parametrize('apr', [0.01, 0.07, 0.2])
@pytest.mark.parametrize('n', [1, 12, 365])
...
@pytest.mark.parametrize('apr', [0.01, 0.07, 0.2])
@pytest.mark.parametrize('n', [2, 12, 365])
...
@pytest.mark.parametrize('n', [1, 6, 365])
def test_effective_rate_positive(nominal, n):
    """Effective rate should be positive."""
    eff = effective_interest_rate(nominal, n)
    assert eff > 0

@pytest.mark.parametrize('nominal', [0.01, 0.05, 0.15])
@pytest.mark.parametrize('inflation', [0.005, 0.03, 0.1])
...
@pytest.mark.parametrize('rate', [0.01, 0.07, 0.2])
...
@pytest.mark.parametrize('principal', [100, 10000, 100000])
@pytest.mark.parametrize('rate', [0.01, 0.1, 0.15])
@pytest.mark.parametrize('n', [1, 12, 365])
...
@pytest.mark.parametrize('principal', [100, 10000, 100000])
@pytest.mark.parametrize('time', [1, 10, 20])
@pytest.mark.parametrize('n', [1, 12, 365])
def test_compound_monotonic_rate(principal, time, n):
    """Compound interest should grow monotonically with rate."""
    values = [compound_interest(principal, r, time, n) for r in [0.01, 0.03, 0.05, 0.1, 0.15]]
    for i in range(len(values) - 1):
        assert values[i] <= values[i + 1] + 0.01
...
def test_compound_monotonic_n(principal, rate, time):
    ...
    values = [compound_interest(principal, rate, time, n) for n in [1, 2, 4, 12, 52, 365]]
    for i in range(len(values) - 1):
        assert values[i] <= values[i + 1] + 0.01

@pytest.mark.parametrize('principal', [100, 10000, 100000])
@pytest.mark.parametrize('rate', [0.01, 0.05, 0.1])
@pytest.mark.parametrize('time', [0.5, 5, 10])
@pytest.mark.parametrize('n', [1, 12, 365])
