...
def test_black_scholes_call_values(
    S: float, K: float, r: float, sigma: float, T: float, expected: float
):
...
def test_black_scholes_put_values(
    S: float, K: float, r: float, sigma: float, T: float, expected: float
):
...
# Breakeven Tests
@pytest.mark.parametrize(
    "K,premium,option_type,expected",
    [
        (100, 5, "call", 105),
        (100, 5, "put", 95),
    ],
