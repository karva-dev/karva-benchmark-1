...
import random
import pytest
from finlib.montecarlo import geometric_brownian_motion, simulate_paths, simulate_portfolio, monte_carlo_var, monte_carlo_option_price, monte_carlo_asian_option, monte_carlo_barrier_option, simulate_retirement
S0_VALS = [50, 200]
MU_VALS = [-0.05, 0.15]
SIGMA_VALS = [0.1, 0.4]
T_VALS = [0.25, 2.0]
...
@pytest.mark.parametrize('S0', [90, 120])
@pytest.mark.parametrize('K', [90, 120])
...
@pytest.mark.parametrize('S0', [90, 110])
@pytest.mark.parametrize('K', [95, 105])
...
def test_asian_option_nonneg(S0, K, sigma, option_type, seed):
    ...
    result = monte_carlo_asian_option(S0, K, 0.05, sigma, 1.0, 20, option_type, 200, seed=seed)
    assert result['price'] >= 0
...
def test_asian_option_leq_european(S0, K, sigma, option_type, seed):
    ...
    asian = monte_carlo_asian_option(S0, K, 0.05, sigma, 1.0, 20, option_type, 500, seed=seed)
    european = monte_carlo_option_price(S0, K, 0.05, sigma, 1.0, option_type, 500, seed=seed)
    assert asian['price'] <= european['price'] + 5.0

@pytest.mark.parametrize('S0', [90, 110])
@pytest.mark.parametrize('K', [95, 105])
@pytest.mark.parametrize('sigma', [0.2, 0.4])
@pytest.mark.parametrize('barrier_type', ['up-and-out', 'down-and-in'])
...
def test_barrier_option_nonneg(S0, K, sigma, barrier_type, seed):
    ...
    result = monte_carlo_barrier_option(S0, K, barrier, 0.05, sigma, 1.0, 20, 'call', barrier_type, 200, seed=seed)
    assert result['price'] >= 0

@pytest.mark.parametrize('S0', [90, 120])
@pytest.mark.parametrize('mu', [0.0, 0.15])
...
@pytest.mark.parametrize('initial_savings', [50000, 500000])
@pytest.mark.parametrize('annual_return', [0.04, 0.08])
@pytest.mark.parametrize('volatility_val', [0.1, 0.2])
@pytest.mark.parametrize('seed', [1, 5])
def test_retirement_sim_valid(initial_savings, annual_return, volatility_val, seed):
    ...
    result = simulate_retirement(initial_savings, 10000, annual_return, volatility_val, 20, 25, 40000, 100, seed=seed)
    assert 0 <= result['probability_of_ruin'] <= 1
    assert result['mean_final_value'] >= 0 or True
