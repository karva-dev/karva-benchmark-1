...
import random
import pytest
from finlib.risk import volatility, downside_volatility, beta, alpha, sharpe_ratio, sortino_ratio, treynor_ratio, max_drawdown, drawdown_series, calmar_ratio, parametric_var, historical_var, conditional_var, upside_potential_ratio, omega_ratio, gain_loss_ratio, win_rate, stress_test
...
@pytest.mark.parametrize('n_assets', [2, 5])
@pytest.mark.parametrize('n_scenarios', [1, 5])
@pytest.mark.parametrize('seed', [1, 20])
@pytest.mark.parametrize('shock_scale', [0.01, 0.2])
def test_stress_test_valid(n_assets, n_scenarios, seed, shock_scale):
    ...
    scenarios = [[random.gauss(0, shock_scale) for _ in range(n_assets)] for _ in range(n_scenarios)]
